...
def load_auto_config(root: Path) -> AutoConfig:
    ...
    return AutoConfig(
        ...
        job_sites=parse_env_list(values.get("AUTO_JOB_SITES", "all")),
        search_location=(values.get("AUTO_SEARCH_LOCATION", "remote") or "remote").strip(),
        results_wanted=parse_env_int(values.get("AUTO_RESULTS_WANTED", "1500"), default=1500, minimum=50, maximum=5000),
        filter_profile=(values.get("AUTO_FILTER_PROFILE", "") or "").strip(),
...
def save_auto_config(root: Path, config: AutoConfig) -> Path:
    ...
    content = "\n".join(
        [
            ...
            f"AUTO_FILTER_PROFILE={quote_env(config.filter_profile)}",
